...
?>
  ...
  <div class="bg-white border rounded-xl overflow-x-auto">
    <table class="w-full text-sm">
      <thead><tr class="border-b" style="background:#EEF2F6"><th class="text-left px-4 py-3 font-medium text-muted-foreground">ID</th><th class="text-left px-4 py-3 font-medium text-muted-foreground">Название</th><th class="text-left px-4 py-3 font-medium text-muted-foreground hidden sm:table-cell">Категория</th><th class="text-left px-4 py-3 font-medium text-muted-foreground hidden sm:table-cell">Автор</th><th class="text-left px-4 py-3 font-medium text-muted-foreground hidden sm:table-cell">Статус</th><th class="text-right px-4 py-3 font-medium text-muted-foreground hidden sm:table-cell">Дата</th><th class="text-right px-4 py-3 font-medium text-muted-foreground">Действия</th></tr></thead>
      <tbody>
          ...
          <tr class="border-b hover:bg-muted/20">
            ...
            <td class="px-4 py-3 text-right text-xs whitespace-nowrap">
              <?php if ($l['status'] === 'blocked'): ?>
                <form method="post" class="inline"><?= csrf_field() ?><input type="hidden" name="action" value="admin_toggle_listing"><input type="hidden" name="id" value="<?=$l['id']?>"><input type="hidden" name="filter" value="<?=h($filter)?>"><button type="submit" style="background:#16A34A;color:#fff;border:0;border-radius:10px;padding:0.5rem 0.875rem;font-size:0.8125rem;font-weight:700;cursor:pointer">Разблокировать</button></form>
              <?php elseif ($l['status'] === 'active' || $l['status'] === 'pending'): ?>
                <form method="post" class="inline"><?= csrf_field() ?><input type="hidden" name="action" value="admin_toggle_listing"><input type="hidden" name="id" value="<?=$l['id']?>"><input type="hidden" name="filter" value="<?=h($filter)?>"><button type="submit" style="background:#F59E0B;color:#fff;border:0;border-radius:10px;padding:0.5rem 0.875rem;font-size:0.8125rem;font-weight:700;cursor:pointer" title="Отключить объявление">Отключить</button></form>
              <?php endif; ?>
              <form method="post" class="inline" onsubmit="return confirm('Удалить объявление безвозвратно?')"><?= csrf_field() ?><input type="hidden" name="action" value="admin_delete_listing"><input type="hidden" name="id" value="<?=$l['id']?>"><input type="hidden" name="filter" value="<?=h($filter)?>"><button type="submit" style="background:#DC2626;color:#fff;border:0;border-radius:10px;padding:0.5rem 0.875rem;font-size:0.8125rem;font-weight:700;cursor:pointer" title="Удалить безвозвратно">Удалить</button></form>
            </td>
          </tr>
        <?php endforeach; ?>
        ...
        <tbody>
          <?php while ($pm = $promos->fetch()): ?>
            <tr class="border-b hover:bg-muted/20">
              ...
              <td class="px-4 py-3 text-center"><span class="text-xs px-2 py-0.5 rounded-full <?= $pm['payment_status']==='paid' ? 'bg-green-100 text-green-700' : ($pm['payment_status']==='refunded' ? 'bg-gray-100 text-gray-600' : 'bg-yellow-100 text-yellow-700') ?>"><?=h(pay_status_label($pm['payment_status']))?></span></td>
