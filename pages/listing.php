  ...
  <div class="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
    ...
    <div class="space-y-6 min-w-0">
      ...
      if ($lt === 'property') {
        ...
        foreach(['fishing_type'=>'Тип рыбалки','fishing_method'=>'Метод ловли','group_size_max'=>'Макс. группа','season'=>'Сезон'] as $k=>$l) { if (!empty($item[$k]) || $item[$k]==='0') { $v = $item[$k]; if ($k==='season') $v = ['all_season'=>'Круглый год','summer'=>'Лето','winter'=>'Зима','spring'=>'Весна','autumn'=>'Осень'][$v]??$v; if ($k==='group_size_max') $v = (int)$v . ' чел.'; if ($k==='fishing_type') $v = ['rechnaya'=>'Речная','morskaya'=>'Морская','ozernaya'=>'Озёрная','podlednaya'=>'Подлёдная','splav'=>'Сплав'][$v]??$v; if ($k==='fishing_method') $v = ['spin'=>'Спиннинг','fly'=>'Нахлыст','troll'=>'Троллинг','ice'=>'Зимняя','float'=>'Поплавочная'][$v]??$v; $specs[$l]=$v; } }
