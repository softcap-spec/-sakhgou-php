  ...
  <div class="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
    ...
    <div class="space-y-6 min-w-0">
      ...
      if ($lt === 'property') {
        ...
        foreach(['fishing_type'=>'Тип рыбалки','fishing_method'=>'Метод ловли','fish_species'=>'Рыба','boat_type'=>'Тип лодки','max_guests'=>'Макс. гостей','tour_duration_hours'=>'Длительность, ч','meeting_point'=>'Место встречи','what_to_bring'=>'Что взять','group_size_max'=>'Макс. группа','season'=>'Сезон','transfer'=>'Трансфер'] as $k=>$l) { if (!empty($item[$k]) || $item[$k]==='0') { $v = $item[$k]; if ($k==='season') $v = ['all_season'=>'Круглый год','summer'=>'Лето','winter'=>'Зима','spring'=>'Весна','autumn'=>'Осень'][$v]??$v; if ($k==='group_size_max'||$k==='max_guests') $v = (int)$v . ' чел.'; if ($k==='tour_duration_hours') $v = (int)$v . ' ч.'; if ($k==='fishing_type') $v = ['rechnaya'=>'Речная','morskaya'=>'Морская','ozernaya'=>'Озёрная','podlednaya'=>'Подлёдная','splav'=>'Сплав'][$v]??$v; if ($k==='fishing_method') $v = ['spin'=>'Спиннинг','fly'=>'Нахлыст','troll'=>'Троллинг','ice'=>'Зимняя','float'=>'Поплавочная'][$v]??$v; if ($k==='transfer') $v = ['yes'=>'Да','no'=>'Нет','possible'=>'Возможен'][$v]??$v; $specs[$l]=$v; } }
        ...
        foreach(['gear_type'=>'Тип снаряжения','sizes'=>'Размеры','gear_condition'=>'Состояние','max_guests'=>'Макс. гостей','deposit_amount'=>'Депозит'] as $k=>$l) { if (!empty($item[$k])) { $v=$item[$k]; if ($k==='gear_condition') $v = ['new'=>'Новое','excellent'=>'Отличное','good'=>'Хорошее','used'=>'Б/у','satisfactory'=>'Удовлетворительное'][$v]??$v; if ($k==='max_guests') $v=(int)$v.' чел.'; if ($k==='deposit_amount') $v=number_format((float)$v,0,',',' ').' ₽'; $specs[$l]=$v; } }
      } elseif ($lt === 'car_rental') {
        foreach(['car_brand'=>'Марка','car_year'=>'Год','car_type'=>'Тип','transmission'=>'Коробка','seats'=>'Мест','fuel'=>'Топливо','mileage'=>'Пробег, км','requirements'=>'Требования','deposit_amount'=>'Депозит'] as $k=>$l) { if (!empty($item[$k]) || $item[$k]==='0') { $v=$item[$k]; if ($k==='transmission') $v = ['auto'=>'Автомат','manual'=>'Механика','robot'=>'Робот','variator'=>'Вариатор'][$v]??$v; if ($k==='seats') $v=(int)$v.' мест'; if ($k==='mileage') $v=(int)$v.' км'; if ($k==='deposit_amount') $v=number_format((float)$v,0,',',' ').' ₽'; $specs[$l]=$v; } }
      }
      ?>
      <?php
      // «Что включено» хранится либо JSON-списком (мастер), либо простым текстом (форма редактирования)
      $incRaw = $item['includes'] ?? '';
      $incDecoded = is_string($incRaw) ? json_decode($incRaw, true) : null;
      if (is_string($incRaw) && trim($incRaw) !== '' && !is_array($incDecoded)) {
        $specs['Что включено'] = trim($incRaw);
      }
      ?>
      <?php if (!empty($specs)): ?>
      <div class="bg-white border border-[#EBEEF2] rounded-xl p-5">
