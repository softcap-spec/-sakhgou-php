...
function get_user_bookings(int $user_id): array {
  $pdo = db();
  return $pdo->query("SELECT b.*, l.title AS listing_title, l.listing_type, l.location, u.name AS host_name FROM bookings b JOIN listings l ON b.listing_id = l.id JOIN users u ON b.host_id = u.id WHERE b.guest_id = $user_id ORDER BY b.created_at DESC LIMIT 20")->fetchAll();
}

function get_host_bookings(int $user_id): array {
  $pdo = db();
  return $pdo->query("SELECT b.*, l.title AS listing_title, u.name AS guest_name FROM bookings b JOIN listings l ON b.listing_id = l.id JOIN users u ON b.guest_id = u.id WHERE b.host_id = $user_id ORDER BY b.created_at DESC LIMIT 20")->fetchAll();
}
